...
void printSideways(const byte *text, word x, word y, byte textColor, byte bgColor) {
...
void initHud(void) __z88dk_fastcall {
  ...
  print("CHARGE", ENERGY_X + 22, 1, HUD_WHITE, HUD_BLACK);
  print("SCORE", SCORE_X, 1, HUD_WHITE, HUD_BLACK);
  print("HIGH", HISCORE_X + 2, 1, HUD_WHITE, HUD_BLACK);
  print("HEALTH", HEALTH_X + 22, 1, HUD_WHITE, HUD_BLACK);

  printSideways("CHARGING", 1, 100, HUD_WHITE, HUD_BLACK);
  printSideways("FIRE RATE", 1, 200, HUD_WHITE, HUD_BLACK);

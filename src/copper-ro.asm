...
; Read-only half of the copper module: the two per-frame kernels are pure code
; with no self-modifying operands, so they live in bank 28 and cost the main bank
; nothing. Their group counters are mutable, so they sit in copper-rw.asm; a
; `ld a,(nn)` costs the same either side of the split.

EXTERN _random16
EXTERN pf_count, ff_count
